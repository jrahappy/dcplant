...
from .models import Case, Patient, Comment, CaseImage, Category
...
class PatientForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'insurance_info', 'consent_given'
        ]
        widgets = {
            'mrn': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'MRN-XXXX'}),
...
class CaseForm(forms.ModelForm):
    ...
    class Meta:
        model = Case
        fields = [
            'patient', 'category', 'chief_complaint', 'clinical_findings',
            'diagnosis', 'treatment_plan', 'prognosis', 'status', 'priority',
            'assigned_to', 'tags', 'is_shared', 'share_with_branches', 'is_deidentified'
        ]
        widgets = {
            ...
            'share_with_branches': forms.SelectMultiple(attrs={'class': 'form-select', 'size': 3}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and hasattr(user, 'profile'):
            ...
            self.fields['assigned_to'].label_from_instance = lambda obj: f"Dr. {obj.get_full_name() or obj.username}"
...
class CaseImageForm(forms.ModelForm):
    """Form for uploading case images"""
    class Meta:
        model = CaseImage
        fields = [
            'image', 'image_type', 'title', 'description',
            'taken_date', 'is_primary', 'is_deidentified'
        ]
        widgets = {
            'image': forms.FileInput(attrs={'class': 'form-control', 'accept': 'image/*,.dcm,.dicom,application/dicom'}),
...
class MultipleImageUploadForm(forms.Form):
    ...
    images = MultipleFileField(
        required=True,
        help_text='Select multiple files at once using Ctrl/Cmd + Click'
    )
    image_type = forms.ChoiceField(
        choices=CaseImage.IMAGE_TYPE_CHOICES,
        widget=forms.Select(attrs={'class': 'form-select'}),
        initial='PHOTO',
        help_text='This type will be applied to all uploaded images'
    )
    ...
    taken_date = forms.DateField(
        ...
        required=False,
        help_text='Date when images were taken'
    )
    is_deidentified = forms.BooleanField(
        widget=forms.CheckboxInput(attrs={'class': 'form-check-input'}),
        required=False,
        label='Images are de-identified'
    )
