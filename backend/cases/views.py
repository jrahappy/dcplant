...
from .forms import CaseForm, PatientForm, CommentForm, CaseImageForm, CaseFilterForm, CategoryForm, MultipleImageUploadForm
...
def case_list(request):
    ...
    cases = Case.objects.filter(organization=user_org).select_related(
...
def case_detail(request, pk):
    ...
    # Get related data
    comments = case.comments.select_related('author').prefetch_related('mentions', 'replies')
    images = case.images.select_related('uploaded_by')
...
def patient_delete(request, pk):
    ...
    if patient.cases.exists():
        messages.error(request, 'Cannot delete patient with existing cases. Please delete or reassign cases first.')
        return redirect('cases:patient_detail', pk=patient.pk)
...
def image_upload(request, case_pk):
            ...
            for index, uploaded_file in enumerate(files, 1):
                try:
                    ...
                    if len(files) > 1:
                        ...
                        title = title_prefix if title_prefix != 'Image' else uploaded_file.name.split('.')[0]
                    ...
                    image = CaseImage(
                        ...
                        is_primary=(index == 1 and not case.images.filter(is_primary=True).exists())
                    )
    ...
    return render(request, 'cases/image_upload.html', context)
...
def image_edit(request, pk):
    ...
    # Check permissions
    if not (request.user == image.uploaded_by or profile.is_admin or request.user.is_staff):
...
def image_list(request, case_pk):
    ...
    profile = ensure_user_profile(request.user)
    case = get_object_or_404(Case, pk=case_pk, organization=profile.organization)
    images = case.images.select_related('uploaded_by').order_by('-is_primary', '-created_at')
    
    context = {
        'case': case,
        'images': images,
    }
    return render(request, 'cases/image_list.html', context)
...
def dicom_viewer(request, pk):
    ...
    return render(request, 'cases/dicom_viewer.html', context)
...
def dicom_series_viewer(request, pk):
    ...
    dicom_images = case.images.filter(is_dicom=True).order_by('image')
    ...
    return render(request, 'cases/dicom_series_viewer.html', context)
...
def download_dicom_series(request, pk):
    ...
    if not dicom_images.exists():
        return JsonResponse({'error': 'No DICOM images found for this case'}, status=404)
